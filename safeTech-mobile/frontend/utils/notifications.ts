...
Notifications.setNotificationHandler({
  handleNotification: async () => ({
    ...
    shouldShowList: true,
    shouldShowAlert: true, // Add this line
  }),
...
export async function scheduleMedicationReminder(medication: Medication): Promise<string | undefined> {
  ...
  try {
    for (const time of medication.times) {
      ...
      console.log('Scheduled Medication Reminder ID:', identifier);
      return identifier;
    }
  } catch (error) {
...
export async function scheduleRefillReminder(medication: Medication): Promise<string | undefined> {
  ...
  try {
    if (medication.currentSupply <= medication.refillAt) {
      ...
      console.log('Scheduled Refill Reminder ID:', identifier);
      return identifier;
    }
  } catch (error) {
  ...
  try {
    ...
    for (const notification of scheduledNotifications) {
      ...
      if (data?.medicationId === medicationId) {
        ...
        console.log(`Cancelled notification for medication ID: ${medicationId}`);
      }
    }
  } catch (error) {
